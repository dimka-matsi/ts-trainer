...
export function CourseHeader({ course, route }: { course: Course; route: Route }) {
  ...
  const where = lesson ? course.regions[lesson.region]!.name : route.view === "course-cards" ? "Карточки" : route.view === "course-exam" ? "Экзамен" : "Карта";
    ...
    <header className="dt-toolbar">
      <div className="dt-toolbar-in">
        ...
        <nav className="dt-htabs" aria-label="Разделы">
          ...
          {tab({ view: "course-cards", course: course.id }, "Карточки", route.view === "course-cards", due)}
        </nav>
