...
export const lesson: WebLesson = {
  ...
  theory: {
    p: [
      ...
      "Nonce — случайное значение, новое на каждый ответ. Сервер пишет его в заголовок и в атрибут `nonce` своих тегов `<script>`. Внедрённый комментарий `<script>` nonce не знает и не выполнится. `strict-dynamic` передаёт доверие дальше: скрипт с правильным nonce может загрузить другие скрипты, и их тоже выполнят. Встроенные обработчики вроде `onclick=\"…\"` и ссылки `javascript:` под такой политикой не работают, поэтому их переносят в файлы скриптов. `'unsafe-inline'` рядом с nonce браузер просто игнорирует, а в политике без nonce он разрешает любые встроенные скрипты и отключает защиту от внедрения.",
    ...
    keys: [
      ...
      "`'unsafe-inline'` без nonce отключает защиту. Внедряют через `Report-Only`.",
    ],
  },
  tasks: [
    {
