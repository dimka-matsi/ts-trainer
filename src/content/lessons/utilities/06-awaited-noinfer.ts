...
export const lesson: Lesson = {
  ...
  answer: "`Awaited` рекурсивно разворачивает промисы, как `await`; его часто комбинируют с `ReturnType` для асинхронных функций. `NoInfer` (TypeScript 5.4) блокирует вывод параметра типа из конкретной позиции, чтобы вторичный аргумент не расширял тип.",
  theory: {
    p: [
      ...
      "`NoInfer<T>` (TypeScript 5.4) запрещает выводить параметр типа из конкретного аргумента. Без него TypeScript может расширить `T` за счёт опечатки во «вторичном» параметре; с ним опечатка становится ошибкой.",
    ],
...
const r = pick(["a", "b"], "c");`,
      ...
      a: 1,
      why: "`T` выводится из обоих аргументов, и опечатка `\"c\"` добавляется в union. `NoInfer` нужен, чтобы такая опечатка давала ошибку.",
    },
    {
      type: "code",
      kind: "write",
