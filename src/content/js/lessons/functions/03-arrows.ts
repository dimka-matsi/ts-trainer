...
export const lesson: WebLesson = {
  ...
  answer:
    "У стрелочной функции нет своего `this`: она берёт `this` из окружения, где создана, и `call`, `apply` или `bind` его не меняют. У неё нет `arguments`, её нельзя вызвать через `new`, и у неё нет свойства `prototype`. Поэтому стрелки удобны для колбэков внутри методов — они видят `this` метода. А вот как методы объекта их не используют: `this` у такого метода будет внешним, а не объектом.",
  theory: {
    p: [
...
try {
  ...
  console.log(e.name); // стрелку нельзя вызвать через new
    ...
    keys: [
      ...
      "У стрелки нет `arguments` и `prototype`, её нельзя вызвать через `new`.",
    ],
  },
  tasks: [
    {
...
console.log(bound());`,
    ...
    },
    {
      ...
      why: "Обе — функции и замыкания. Отличия стрелки: нет своего `this`, `arguments` и `prototype`, нельзя `new`.",
    },
    {
      type: "quiz",
